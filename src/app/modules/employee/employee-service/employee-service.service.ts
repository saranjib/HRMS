...
export class EmployeeServiceService {
  ...
  getListEmployee2(): Observable<any> {
    return this.httpService.fetchEmpPersDtlId().pipe(
      map((response: any) => {
        const empPersDtlId = response.empPersDtlId;
        // Use empPersDtlId to fetch employee list
        return this.httpService.makeGetReq(this.configuration.getEmp);
      })
    );
  }
...
submitToEmailService(emailServiceData:any){
  debugger
  return this.httpService
  // .makeGetReq(this.configuration.getEmp, payLoad )
  .makePostReq2(emailServiceData, this.configuration.sendemail)
  .pipe(
    map((res: any) => {
      return res;
